...
class Lane():
    ...
    def result_decorated(self):
        ...
        result = self.result
        result = cv2.putText(result, '%0.2f,%0.2f' % (cl, clm), (5, 15), font, .5, (255, 255, 255), 2, cv2.LINE_AA)
        result = cv2.putText(result, '%0.2f,%0.2f' % (cr, crm), (200, 15), font, .5, (255, 255, 255), 2, cv2.LINE_AA)
        result = cv2.putText(result, '%.3f' % xcoffset, (150, 220), font, .5, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    def x_error(self):
        lx = self.x_start_left
        rx = self.x_start_right
        xcenter = self.warped.shape[1] / 2
        self.error = xcenter - (rx+lx)/2
        return self.error
